...
exports.updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  const oldAvatar = await fs.unlink(avatarLocalPath);

  if (!oldAvatar) {
    throw new ApiError(400, "Error while deleting on avatar");
  }
  ...
  const channel = await User.aggregate([
    ...
    },
    {
      $addFields: {
        ...
        isSubscribed: {
          $cond: {
            if: { $in: [req.user?._id, "$subscribers.subscriber"] }, // $in: used for check array also objects
            then: true,
            else: false,
          },
        },
...
exports.getWatchHistory = asyncHandler(async (req, res) => {s
